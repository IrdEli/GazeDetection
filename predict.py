...
out = imageio.get_writer('GazeConvo2MP.mp4')#,fps=cv2.CAP_PROP_FPS)
color_encoding = []
...
for i in range(0, len(v)-4): 
    ...
    for j in range(i-3, i+4):
      #print(i+j)
      #print(x0, x1, y0, y1, height, width)
      face_image = v[j][bbx_all[j][2]:bbx_all[j][3], bbx_all[j][0]:bbx_all[j][1]]
      #plt.imshow(face_image)
      #plt.savefig('{}/{}.jpg'.format(i,j))
    ...
    gaze = spherical2cartesial(output_gaze).detach().numpy()
    print(i, gaze)
    eye = np.asarray(eye).astype(float)
    #print(eye)
    ...
    image = cv2.rectangle(image, (bbx[0],bbx[2]), (bbx[1],bbx[3]),color_encoding[900])
    #image = image.astype(float)
    #print(bbx[0], bbx[2], bbx[1], bbx[3])
    #image = image.astype(float)
    #image = image.astype(np.uint8)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    ...
    #print(count)
    #if(x0 >0 and x1>0 and y1>0 and y1>0):
      ##cv2.imshow('MediaPipe Face Detection', face_image)
...
cap = cv2.VideoCapture('GazeConvo.mp4')
length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
fps = cap.get(cv2.CAP_PROP_FPS)
height, width, channels =image.shape
