...
def cartesial2spherical(x):
    output = [0,0,0]
    #print('Input to the function: ',x)
    hxy = np.hypot(x[0], x[1])
    output[0] = np.hypot(hxy,x[2])
    output[2] = math.degrees(np.arctan2(hxy,x[2]))
    output[1] = math.degrees(np.arctan2(x[1], x[0]))
    #print('Output of fuction:', output)
    
    #output[0] = math.sqrt(x[0]*x[0]+x[1]*x[1]+x[2]*x[2])
    #output[1] = math.degrees(math.atan(x[1]/x[0]))
    #output[2] = math.degrees(math.acos(x[2]/(math.sqrt(x[0]*x[0]+x[1]*x[1]+x[2]*x[2]))))
...
def detectGaze(v, bbx_all,eyes ,noPPF, eye_position, x_rot, y_rot,z_rot):
    ...
    #print(gaze)
    eye_pos_deg = cartesial2spherical(eye_position)
    converted_output_gaze = cartesial2spherical(gaze[0])
    output_gaze = output_gaze.cpu().detach().numpy()#cartesial2spherical(gaze[0]) #
    #print(output_gaze,gaze, converted_output_gaze)
    #print(cartesial2spherical(gaze[0]), math.degrees(output_gaze[0][1]), math.degrees(output_gaze[0][0]))
    gaze_deg = ([math.degrees(output_gaze[0][0]),math.degrees(output_gaze[0][1])]) 
    
    diff = []
    #check which gaze output is which axes
    #if (eye_pos_deg[1]<0):
        #eye_pos_deg[1] = -1*(90+eye_pos_deg[1])
    #Head Rotation mid value = x-axis left is positive , right is negative
    #Head Rotation first value = y-axis top is positive, bottom is negative
    #EyeGaze firstvalue = x-axis, left is negative, right is positive
    #Eyegaze 2nd value = y-axis top is positive , bottom is negative
    #camera first value x-axis, right side is positive, left side is negative
    #camera 2nd value top side is positive and bottom side is negative (assumption not verified)
    #print('First Row:' ,((gaze_deg[1])+(eye_pos_deg[1])-y_rot))
    #print('Second Row:' ,((gaze_deg[0])-(eye_pos_deg[2])+x_rot))
    #print(eye_position)
    diff.append(((gaze_deg[0])+ y_rot)) #-(eye_pos_deg[1])
    diff.append(((gaze_deg[1])+ x_rot)) #-(eye_pos_deg[2])  
    ...
    if diff[0]>robApos[0]-errorRate and diff[0]<robApos[0]+errorRate and diff[1]>robApos[1]-errorRate and diff[1]<robApos[1]+errorRate:
        print("Looking Robot A: ", robApos[1])
        look = 0
    elif diff[0]>robBpos[0]-errorRate and diff[0]<robBpos[0]+errorRate and diff[1]>robBpos[1]-errorRate and diff[1]<robBpos[1]+errorRate:
        print("Looking Robot B")
        look = 1
    else:
        print("Not Looking")
        look = -1
    ...
    if look == 0:
        ...
        image = cv2.circle(image, (10,10), 14, (0,0,255))
        
    else:
        image = cv2.circle(image, (10,10), 14, (255,0,)) 
    #print(output_gaze)
    cv2.putText(image,'Difference: '+ np.array2string(np.asarray(diff)),(30,140), font, fontScale, fontColor,thickness, lineType)
    cv2.putText(image,'Head Rotation: '+ np.array2string(np.asarray([x_rot,y_rot])), (30,30), font, fontScale, fontColor,thickness, lineType)
    cv2.putText(image, 'From Eye: ' + np.array2string(np.asarray(gaze_deg)),(30,70), font, fontScale, fontColor,thickness, lineType)
    #cv2.putText(image, 'From Eye: ' + np.array2string(np.asarray(converted_output_gaze)),(30,70), font, fontScale, fontColor,thickness, lineType)
    cv2.putText(image, 'From Camera:'+np.array2string(np.asarray(eye_pos_deg[1:3])),(30,110), font, fontScale, fontColor,thickness, lineType)
...
def recData(data, payload_size):
    ...
    data = data[msg_size:]
    depth, frame, eye_position = pickle.loads(frame_data) 
    return depth, frame, eye_position

def getFaceRot(frame):
...
host_ip = '192.168.1.2'#'192.168.0.19'
port = 9123
...
while True:
    #depth, frame, eye_position = recData(data, payload_size) #retreive the depth and color information from the realsense
    ...
    frame_data = data[:msg_size]
    data = data[msg_size:]
    frame, eye_position = pickle.loads(frame_data) 
