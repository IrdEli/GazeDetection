...
def cartesial2spherical(x):
    ...
    output[1] = math.atan2(x[1],x[0])
    output[2] = math.atan(math.sqrt(x[0]*x[0]+x[1]*x[1])/x[2])
    return output

def detectGaze(v, bbx_all,eyes ,noPPF, eye_position):
    ...
    print(output_gaze)
    #print(i, output_gaze, eye_position_spherical[1:3], output_gaze -eye_position_spherical[1:3]) 
    diff = []
    diff.append(output_gaze[0][0]*(-1)-abs(eye_position_spherical[1]))
    diff.append(output_gaze[0][1]*(-1)-abs(eye_position_spherical[2]))    
    
    look = False
    if(diff[0]< 0.3 and diff[0]>-0.3 and diff[1]<0.3 and diff[1]>-0.3):
        #print("Looking")
        look = True
    else:
        #print("Not Looking")
    ...
    cv2.putText(image, ''.join(map(str,diff)),topCorner, font, fontScale, fontColor,thickness, lineType)
    cv2.putText(image, np.array2string(output_gaze),(60,100), font, fontScale, fontColor,thickness, lineType)
    #if(len(eye_position_spherical)!=0):
    cv2.putText(image, ''.join(map(str,eye_position_spherical[1:3])),(90,150), font, fontScale, fontColor,thickness, lineType)
        ...
        if results.detections:
          ...
          #mp_drawing.draw_detection(image, detection)
            ...
            #run the algorithm after the first 6 frames
                    #print('inside', len(v))
                    detectGaze(v, bbx_all, eyes, noPPF, eye_position)
                    v.pop(0)
                #print('x')
